...
import cn.biggar.biggar.presenter.update.FansPresenter;
...
public class FansActivity extends BiggarListActivity<FansPresenter, FansListBean> implements FansContract.View {
    ...
    protected void initDataBefore() {
        ...
        if (TextUtils.isEmpty(mUserId) || !mUserId.equals(mCid)) {
            RootLayout.getInstance(this).setTitle("Ta的粉丝");
        } else {
            RootLayout.getInstance(this).setTitle("粉丝");
        }
    }
    ...
    protected void refreshData(boolean isLoadMore) {
        mPresenter.requestFans(mUserId, mCid, curPage, Constants.PAGE_SIZE, isLoadMore);
    }
    ...
    protected void onListItemChildClick(int viewId, FansListBean data, int position) {
        switch (viewId) {
            case R.id.iv_tochat:
                ...
                if (userBean == null || TextUtils.isEmpty(mUserId)) {
                    startActivity(LoginActivity.startIntent(FansActivity.this));
                    return;
                }
     ...
     * 取消关注
     */
    private void cancelFollow(final FansListBean fansListBean) {
        showLoading();
        mPresenter.cancelFollow(mUserId, fansListBean.getID());
    }
     ...
     * @param fansListBean
     */
    private void addFollow(FansListBean fansListBean) {
        showLoading();
    ...
    @Override
    public void showFans(List<FansListBean> fansListBeen) {
        finishRefresh(fansListBeen);
    }

    @Override
    public void showMoreFans(List<FansListBean> fansListBeen) {
