...
/**
 * Created by mx on 2016/9/9.
 * textview layout  如搜索记录等
 */
public class MultipleTextViewGroup extends RelativeLayout {
    ...
    /**
     * 自己加的    源码  刷新有BUG  强制重新加载   或许有别的办法
     * @param dataList
     */
    public void setTextViews2(final List<String> dataList) {
            this.removeAllViews();
            ViewTreeObserver viewTreeObserver = getViewTreeObserver();
            if (viewTreeObserver.isAlive()) {
                viewTreeObserver.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
...
//            tv.setEllipsize(TextUtils.TruncateAt.END);
            ...
            if (x + tvw+textPaddingLeft+textPaddingRight+ wordMargin > layout_width || lineMap.get(line).size() >= columnNum) {
                x = 0;
                y = y + tvh + lineMargin;
